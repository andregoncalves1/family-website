<!-- frontend/src/components/FeverMedicationChart.vue -->
<template>
    <v-card>
    ...
    setup(props) {
      ...
      const fetchData = async () => {
        try {
          ...
          this.$toast.error('Erro ao carregar dados do gráfico.')
      ...
      const prepareChart = (records, medications, thresholds) => {
        ...
        const sortedRecords = records.sort((a, b) => new Date(a.date_time) - new Date(b.date_time))
      ...
      watch(() => [props.startDate, props.endDate, props.diseaseID], () => {
        fetchData()
      })
